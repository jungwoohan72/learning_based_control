...
for update in range(1000000):
    ...
    if update % cfg['environment']['eval_every_n'] == 0:
        ...
        # or total_steps == 10000000
    # if average_performance > -2.5 and first_flag == True and update > 1900:
    #     torch.save({
    #         'actor_architecture_state_dict': actor.architecture.state_dict(),
    #         'actor_distribution_state_dict': actor.distribution.state_dict(),
    #         'critic_architecture_state_dict': critic.architecture.state_dict(),
    ...
    #     critic = ppo_module.Critic(ppo_module.MLP(cfg['architecture']['value_net'], nn.LeakyReLU, ob_dim, 1),
    #                                device)
    #     actor.architecture.load_state_dict(torch.load(saver.data_dir+"/full_"+str(update)+'.pt')['actor_architecture_state_dict'])
    #     actor.distribution.load_state_dict(torch.load(saver.data_dir+"/full_"+str(update)+'.pt')['actor_distribution_state_dict'])
    #     critic.architecture.load_state_dict(torch.load(saver.data_dir+"/full_"+str(update)+'.pt')['critic_architecture_state_dict'])
    #
    #     ppo = PPO.PPO(actor=actor,
    #                   critic=critic,
    #                   num_envs=cfg['environment']['num_envs'],
    ...
    #                   )
    #
    #     ppo.optimizer.load_state_dict(torch.load(saver.data_dir+"/full_"+str(update)+'.pt')['optimizer_state_dict'])
    #     first_flag = False
    #
    #     total_iteration_steps = n_steps * env.num_envs
...
print("Visualizing and evaluating the last policy")
torch.save({
    'actor_architecture_state_dict': actor.architecture.state_dict(),
    'actor_distribution_state_dict': actor.distribution.state_dict(),
    'critic_architecture_state_dict': critic.architecture.state_dict(),
    'optimizer_state_dict': ppo.optimizer.state_dict(),
}, saver.data_dir+"/full_"+str(final_update)+'.pt')
...
env.reset()
env.save_scaling(saver.data_dir, str(final_update))